...
describe('read-only release evidence page', () => {
  it('shows measured semantic results and the authenticated runtime revision', async () => {
    ...
    expect(screen.getByRole('status')).toHaveTextContent('完成全部冻结真实语义门禁')
    expect(screen.getByRole('status')).toHaveTextContent('所有者发布报告')
    ...
    expect(
      screen.getByText('5d02d3010b4e5560d9de697b471391e6ff742796'),
    ).toBeInTheDocument()
  })
})
